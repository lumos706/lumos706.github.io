...
`.github/workflows/travel-data.yml` 每天 `01:00 UTC`（北京时间 09:00）运行一次完整刷新。任务会安装官方 skill、FlyAI skill、Miniconda、`jieba` 与 Playwright Chromium，一次性更新攻略、餐馆、当地交通、出发准备、景区预约、机票和酒店，通过生产构建后再提交数据并触发 Pages 部署。预约证据不足时保留上一份有效规则；交通证据不足时保留人工核对过的稳妥基线，不根据单条帖子擅自改写公交线路号。
